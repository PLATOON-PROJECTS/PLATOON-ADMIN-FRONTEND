...
const emit = defineEmits<{
  (e: "close_net"): void;
  (e: "save_net", netPayData: { amount: number }): void;
}>();
...
const rules = computed(() => ({
  amount: {
    required: helpers.withMessage("Netpay amount is required", required),
  },
}));
...
const handleSubmit = () => {
  ...
  emit("save_net", { amount: data.value.amount });
              ...
              <div class="flex items-start justify-between">
                <div>
                  ...
                  <p class="text-sm font-semimedium text-black-rgba-100">
                    NetPay is the employee's salary before deductions.
                  </p>
                </div>
                <button
                  type="button"
                  v-on:click="$emit('close_net')"
